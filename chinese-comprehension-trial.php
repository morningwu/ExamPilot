...
include('function/user.php');
    ...
    $level              = $_GET['level'];
    $questionChinese    = mysql_query("SELECT DISTINCT (`mission_id`),`question_type`,`title` FROM `questions_chinese` WHERE `subject` = 'Chinese' AND `learning_area` = 'Comprehension' AND `level` = '$level' LIMIT 0,6");
                    ...
                    '<div class="span4 filterable assessment-container">
                        <img src="_assets/img/english/vocabulary/vocabulary-icon-'.$photoNum.'.png" alt="Chinese-Vocabulary-Icon"/>
                            ...
                            <table>
                                ...
                                <tr>
                                    <td align="right">題目類別：</td>
                                    <td>';

                                    if($questionType == 'Mutiple Choice'){$questionType = '選擇題'; echo $questionType; }else{$questionType = '填充題'; echo $questionType;}
                                ...
                                <div class="aboutsocial">
                                    <a id="flat-btn" class="flat-button flat-btn-round-3" href="chinese-comprehension-question-trial.php?id='.$missionId.'&assessmenttitle='.$trimmed_title.'&questiontype='.$trimmed_questionType.'&level='.$level.'">做練習 &rarr;</a> 
                                </div>
                            </div>
                    </div>';
