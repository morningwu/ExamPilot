...
function add_questions_maths($mission_id,$level,$learning_area,$question_type,$title,$passage,$questions,$selection1,$selection2,$selection3,$selection4,$answer,$created_by,$date_of_creation){
  ...
  mysql_query("INSERT INTO `questions_maths`(
    `mission_id`, 
    `level`,
    `learning_area`,
    `question_type`,
  ...
  echo '<div class="question_notice">'."Questions has been added (Maths)!Thanks you!".'</div>';
}
...
function questions_exist($questions){
  ...
  $total = mysql_query("SELECT COUNT(`question_id`) FROM `questions_maths` WHERE `questions` = '{$questions}'");
